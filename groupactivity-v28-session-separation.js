/* EnglishForPublicHealth · Group Activity V28 consolidated hardening
...
(() => {
  ...
  const PATCH_VERSION = 'V28-20260915';
...
/* --------------------------------------------------------------------------
   V28 · Clear separation between Sessions 1–7 + concise Session 4 onboarding
   UI-only hardening. No scenario option, score value or option order changes.
   -------------------------------------------------------------------------- */
(() => {
  'use strict';

  const VERSION = 'V28-20260915';
  ...
  const STYLE_ID = 'groupactivity-v28-session-style';
  if (!document.getElementById(STYLE_ID)) {
    ...
    style.textContent = `
      ...
      .v28-s4-steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.6rem}
  ...
  function session4QuickStart() {
    ...
    if (!ws.querySelector('.v28-s4-quickstart')) {
      const box = document.createElement('section');
      box.className = 'v28-s4-quickstart';
      box.setAttribute('aria-label', 'Session 4 quick instructions');
      box.innerHTML = `
        <span class="eyebrow">Session 4 · Quick start</span>
        <h4>Four steps. One shared decision each time.</h4>
        <div class="v28-s4-steps">
          <div class="v28-s4-step"><span>1</span><strong>Choose 3 or 4 students and assign the roles.</strong></div>
          <div class="v28-s4-step"><span>2</span><strong>Read the situation and discuss every option.</strong></div>
          <div class="v28-s4-step"><span>3</span><strong>Select ONE answer together, then confirm it.</strong></div>
          <div class="v28-s4-step"><span>4</span><strong>Read the consequence and say the Pitch Checkpoint before continuing.</strong></div>
        </div>
        <p class="v28-s4-parity"><strong>Device rule:</strong> two groups choosing the same option at the same decision get the same consequence, score change and next screen. Final task: one <strong>2:00 briefing for the whole group</strong>.</p>`;
      const article = ws.querySelector(':scope > article');
      if (article) article.prepend(box);
      else ws.prepend(box);
    }

    // Shorten repeated technical wording in the overview without changing logic.
    ws.querySelectorAll('p').forEach(p => {
      ...
      if (text.startsWith('Time rule: 2:00 is the total for the whole group')) {
        ...
        p.innerHTML = '<strong>Same choices on different devices → same scores and same next screen.</strong>';
      }
    });
  }
